export function initSupplier() {
	...
	$('#supplier-form').on('submit', function (e) {
		...
		$('#success-modal-overlay, #supplier-error-modal-overlay').remove();

		// Collect form data
		var formData = new FormData(this);
		formData.append('action', 'suppliers_registration');
		...
		const actionButtons = (guest) => {
			...
			if (isToday) {
				if (isCompleted) {
					...
					return `<button id="sign-in-supplier-button-${guest.id}" class="${baseButtonClasses} ${isButtonDisabled ? disabledClasses : signInEnabledClasses}" data-visit-id="${guest.visit_id}" ${isButtonDisabled ? 'disabled' : ''}>Sign In</button>`;
				} else if (!guest.sign_out_time) {
					return `<button id="sign-out-supplier-button-${guest.id}" class="${baseButtonClasses} ${isButtonDisabled ? disabledClasses : signOutEnabledClasses}" data-visit-id="${guest.visit_id}" ${isButtonDisabled ? 'disabled' : ''}>Sign Out</button>`;
				}
			}
			...
			success: function (response) {
				if (response.success && response.data.guestData) {
							...
							$('#success-modal-overlay').fadeOut(
								...
								function () {
									...
									window.dispatchEvent(
										new Event('close-supplier-modal')
									);
					...
					const errorMessages = response.data?.messages || [
						'An error occurred during supplier registration',
					];
					const errorMessageHtml = errorMessages
						...
						.join('');
					const errorModal = `
						<div id="supplier-error-modal-overlay" class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-5">
							<div class="bg-white dark:bg-gray-900 border border-white/10 rounded-2xl p-8 shadow-lg text-center animate-fade-in-up max-w-sm w-full">
								<div class="check_mark mx-auto mb-4">
					...
					$(document)
						...
						.on('click', '#ok-error-btn', function (e) {
							e.preventDefault();
							$('#supplier-error-modal-overlay').fadeOut(
								300,
								function () {
									$(this).remove();
								}
	...
	$(document).on('click', '[id^="edit-supplier-button-"]', function (e) {
		e.preventDefault();

		// Extract guest ID from button ID
		const guestId = $(this).attr('id').replace('edit-supplier-button-', '');

		// Redirect to edit page
		window.location.href =
			vms_script_ajax.home_url +
			'/supplier-details/?guest_id=' +
			guestId +
			'&paged=1';
	});

	// Supplier Update FORM
	$('#supplier-update-form').on('submit', function (e) {
		e.preventDefault();
		...
		// Collect form data
		var formData = new FormData(this);
		formData.append('action', 'update_suppliers');
		formData.append('nonce', vms_script_ajax.nonce);
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					...
					const errorMessages = response.data.messages || [
						'An error occurred during supplier update',
					];
					const errorMessageHtml = errorMessages
						.map(
							(msg) =>
					...
					const errorModal = `
                            <div id="supplier-error-modal-overlay"
	...
	$('#delete-supplier-btn').on('click', function (e) {
		...
		const confirmModal = `
			<div id="delete-supplier-confirm-modal-overlay" class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-5">
				<div class="bg-white dark:bg-gray-900 border border-white/10 rounded-2xl p-8 shadow-lg text-center animate-fade-in-up max-w-sm w-full">
					...
					<h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-white">Delete Supplier</h3>
					...
					<div class="flex gap-3">
						<button id="cancel-delete-supplier-btn" type="button" class="flex-1 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">Cancel</button>
						<button id="confirm-delete-supplier-btn" type="button" class="flex-1 rounded-lg bg-red-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-red-600">Delete</button>
					</div>
				</div>
			</div>
		`;
	...
	$(document).on('click', '#cancel-delete-supplier-btn', function () {
		$('#delete-supplier-confirm-modal-overlay').fadeOut(300, function () {
			$(this).remove();
		});
	});

	// Confirm Delete Supplier
	$(document).on('click', '#confirm-delete-supplier-btn', function () {
		// Show loading
		$(this)
			.prop('disabled', true)
			.html(
		...
		formData.append('action', 'delete_suppliers');
		formData.append('nonce', vms_script_ajax.nonce);
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					...
					$('#delete-supplier-confirm-modal-overlay').fadeOut(
						300,
						function () {
							$(this).remove();
						}
					...
					// Show error messages in a single modal
					const errorMessages = response.data.messages || [
						'An error occurred during supplier deletion',
					];
					const errorMessageHtml = errorMessages
						.map(
							(msg) =>
	...
	// VISIT FORM
	$('#supplier-visit-form').on('submit', function (e) {
		e.preventDefault();
		...
		// Collect form data
		var formData = new FormData(this);
		formData.append('action', 'register_supplier_visit');
		formData.append('nonce', vms_script_ajax.nonce);
	...
	// Sign In Supplier Button Handler
	$(document).on('click', '[id^="sign-in-supplier-button-"]', function (e) {
		e.preventDefault();
		...
		const confirmModal = `
			<div id="sign-in-confirm-modal-overlay" class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-5">
				<div class="bg-white dark:bg-gray-900 border border-white/10 rounded-2xl p-8 shadow-lg text-center animate-fade-in-up max-w-sm w-full">
					...
					<h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-white">Sign In Supplier</h3>
					<p class="mb-4 text-sm text-gray-500 dark:text-gray-400">Enter ID Number for "${guestName}" to continue:</p>
					
					<input type="number" id="supplier-id-number" placeholder="Enter ID Number" 
					...
					<div class="flex gap-3">
						...
						<button id="confirm-sign-in-supplier-btn" type="button" 
	...
	$(document).on('input', '#supplier-id-number', function () {
		const idNumber = $(this).val().trim();
		if (idNumber.length >= 5) {
			$('#confirm-sign-in-supplier-btn').prop('disabled', false);
		} else {
			$('#confirm-sign-in-supplier-btn').prop('disabled', true);
		}
	});
	...
	// Confirm Sign In
	$(document).on('click', '#confirm-sign-in-supplier-btn', function () {
		const visitId = $(this).data('visit-id');
		const idNumber = $('#supplier-id-number').val().trim();
		const button = $(
			`[id^="sign-in-supplier-button-"][data-visit-id="${visitId}"]`
		);
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success && response.data.guestData) {
					...
					const signOutBtn = `
					<button id="sign-out-supplier-button-${guest.id}" data-visit-id="${newVisitId}"
					...
					const successMessage =
						response.data.messages?.[0] ||
						'Supplier signed in successfully';
					showSuccessModal(successMessage);
				} else {
					const errorMessage =
						response.data?.messages?.join('<br>') ||
						'Error signing in supplier';
	...
	// Sign Out Supplier Button Handler
	$(document).on('click', '[id^="sign-out-supplier-button-"]', function (e) {
		e.preventDefault();
		...
		// Show confirmation modal
		const confirmModal = `
			<div id="sign-out-confirm-supplier-modal-overlay" class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-5">
				<div class="bg-white dark:bg-gray-900 border border-white/10 rounded-2xl p-8 shadow-lg text-center animate-fade-in-up max-w-sm w-full">
					...
					<h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-white">Sign Out Supplier</h3>
					...
					<div class="flex gap-3">
						...
						<button id="confirm-supplier-sign-out-btn" type="button" class="flex-1 rounded-lg bg-orange-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-orange-600" data-visit-id="${visitId}">Sign Out</button>
					</div>
				</div>
			</div>
		`;
	...
	$(document).on('click', '#cancel-sign-out-btn', function () {
		$('#sign-out-confirm-supplier-modal-overlay').fadeOut(300, function () {
			$(this).remove();
		});
	});

	// Confirm Sign Out
	$(document).on('click', '#confirm-supplier-sign-out-btn', function () {
		const visitId = $(this).data('visit-id');
		const button = $(
			`[id^="sign-out-supplier-button-"][data-visit-id="${visitId}"]`
		);
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success && response.data.guestData) {
					...
					// Close confirm modal
					$('#sign-out-confirm-supplier-modal-overlay').fadeOut(
						300,
						function () {
							$(this).remove();
						}
					...
					const successMessage =
						response.data.messages?.[0] ||
						'Supplier signed out successfully';
					showSuccessModal(successMessage);
				} else {
					const errorMessage =
						response.data?.messages?.join('<br>') ||
						'Error signing out supplier';
					showErrorModal(errorMessage);
	...
	function showErrorModal(message) {
		...
		$(document)
			...
			.on('click', '#ok-error-btn', function (e) {
				e.preventDefault();
				$('#supplier-error-modal-overlay').fadeOut(300, function () {
					$(this).remove();
				});
			});
	}
