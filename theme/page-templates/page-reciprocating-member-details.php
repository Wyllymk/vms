...
if ( wp_doing_ajax() ) {
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] === 'update_recip_member' ) {
		...
		if ( ! in_array( $member_status, array( 'active', 'suspended', 'banned' ) ) ) {
		...
		if ( $updated !== false ) {
			wp_send_json_success( array( 'message' => 'Member updated successfully' ) );
		} else {
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] === 'delete_recip_member' ) {
		...
		$deleted = $wpdb->delete( $recip_members_table, array( 'id' => $member_id ), array( '%d' ) );

		if ( $deleted ) {
			wp_send_json_success(
				array(
					'message'  => 'Member deleted successfully',
					'redirect' => admin_url( 'admin.php?page=reciprocating-members' ),
				)
			);
		} else {
...
// Handle recip member export
if ( isset( $_POST['export_recip_member'] ) && isset( $_POST['member_id'] ) ) {

	// Verify nonce
	if ( ! isset( $_POST['export_recip_member_nonce'] ) ||
		! wp_verify_nonce( $_POST['export_recip_member_nonce'], 'export_recip_member_action' ) ) {
		wp_die( 'Security check failed' );
	}

	$export_type      = sanitize_text_field( $_POST['export_recip_member'] );
	$export_member_id = absint( $_POST['member_id'] );

	if ( $export_member_id !== $member_id ) {
		wp_die( 'Invalid member ID' );
	}
	...
	if ( $export_type === 'pdf' ) {
		VMS_Export_Handler::export_recip_member_pdf( $export_member_id );
	}
						...
						<div
							...
							<div class="flex items-center gap-2">
								...
								<span class="text-sm font-medium text-gray-700 dark:text-gray-400">
									Export Reciprocating Member Details
								</span>
							</div>
							<div class="flex flex-wrap gap-2">
								<!-- PDF Export Button -->
								<form method="post" action="" class="inline-block">
									<input type="hidden" name="export_recip_member" value="pdf">
									<input type="hidden" name="member_id" value="<?php echo esc_attr( $member_id ); ?>">
									<?php wp_nonce_field( 'export_recip_member_action', 'export_recip_member_nonce' ); ?>
									<button type="submit"
										...
										<div x-show="open" x-transition
											...
											<form id="recip-update-form">
												<div class="-mx-2.5 flex flex-wrap gap-y-4">
													...
													<!-- Club Dropdown -->
													<div class="w-full px-2.5 md:w-1/2">
														<label for="reciprocating_club_id"
															class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
															Club:
														</label>
														<div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
															...
															<select id="reciprocating_club_id" name="reciprocating_club_id"
																...
																<?php
																// Get clubs from cache or database
																$transient_key = 'vms_reciprocating_clubs_cache';
																$clubs = get_transient($transient_key);
																
																// If cache doesn't exist or is empty, fetch from DB
																if ($clubs === false || empty($clubs)) {
																	global $wpdb;
																	$recip_clubs_table = VMS_Config::get_table_name(VMS_Config::RECIP_CLUBS_TABLE);
																	$clubs = $wpdb->get_results(
																		$wpdb->prepare(
																			"SELECT id, club_name FROM {$recip_clubs_table} WHERE status = %s ORDER BY club_name ASC",
																			'active'
																		),
																		ARRAY_A
																	);
																	...
																	// Cache for 6 hours
																	if (!empty($clubs)) {
																		set_transient($transient_key, $clubs, HOUR_IN_SECONDS * 6);
																	}
																}
																...
																if (!empty($clubs)) {
																	foreach ($clubs as $club) {
																		$selected = ($club['id'] == ($member->reciprocating_club_id ?? 0)) ? 'selected' : '';
