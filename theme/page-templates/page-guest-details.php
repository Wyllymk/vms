...
$is_allowed   = in_array( 'administrator', $current_user->roles ) || in_array( 'general_manager', $current_user->roles ) || in_array( 'chairman', $current_user->roles ) || in_array( 'reception', $current_user->roles );

// Check if user has member or chairman role
$restricted_roles    = array( 'member', 'chairman' );
$has_restricted_role = ! empty( array_intersect( $restricted_roles, $user_roles ) );

// Build the WHERE clause for host filtering
$host_filter        = '';
$host_filter_params = array( $guest_id );
...
if ( $total_visits > 0 ) {
	$visits_query  = "
        ...
        LIMIT %d OFFSET %d
    ';
	$visits_params = array_merge( $host_filter_params, array( $per_page, $offset ) );
...
// Handle guest export
if ( isset( $_POST['export_guest'] ) && isset( $_POST['guest_id'] ) ) {

	// Verify nonce
	if ( ! isset( $_POST['export_guest_nonce'] ) ||
		! wp_verify_nonce( $_POST['export_guest_nonce'], 'export_guest_action' ) ) {
		wp_die( 'Security check failed' );
	}

	$export_type     = sanitize_text_field( $_POST['export_guest'] );
	$export_guest_id = absint( $_POST['guest_id'] );

	if ( $export_guest_id !== $guest_id ) {
		wp_die( 'Invalid guest ID' );
	}
	...
	if ( ! current_user_can( 'administrator' ) &&
		...
		wp_die( 'You do not have permission to export guest details' );
	}

	// Process export based on type
	if ( $export_type === 'pdf' ) {
		VMS_Export_Handler::export_guest_pdf( $export_guest_id );
	}
                    ...
                    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) min-h-screen md:p-6">
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-400">Export Guest
                                    Details</span>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <form method="post" action="" class="inline-block">
                                    <input type="hidden" name="export_guest" value="pdf">
                                    <input type="hidden" name="guest_id" value="<?php echo esc_attr( $guest_id ); ?>">
                                    <?php wp_nonce_field( 'export_guest_action', 'export_guest_nonce' ); ?>
                                    <button type="submit"
