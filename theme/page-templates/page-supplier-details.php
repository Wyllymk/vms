...
if ( isset( $_POST['export_supplier'] ) && isset( $_POST['supplier_id'] ) ) {

	// Verify nonce
	if ( ! isset( $_POST['export_supplier_nonce'] ) ||
		! wp_verify_nonce( $_POST['export_supplier_nonce'], 'export_supplier_action' ) ) {
		wp_die( 'Security check failed' );
	}

	$export_type        = sanitize_text_field( $_POST['export_supplier'] );
	$export_supplier_id = absint( $_POST['supplier_id'] );

	if ( $export_supplier_id !== $guest_id ) {
	...
	if ( ! current_user_can( 'administrator' ) &&
		...
		wp_die( 'You do not have permission to export supplier details' );
	}

	// Process export based on type
	if ( $export_type === 'pdf' ) {
		VMS_Export_Handler::export_supplier_pdf( $export_supplier_id );
	}
						...
						<div
							...
							<div class="flex flex-wrap gap-2">
								...
								<form method="post" action="" class="inline-block">
									<input type="hidden" name="export_supplier" value="pdf">
									<input type="hidden" name="supplier_id" value="<?php echo esc_attr( $guest_id ); ?>">
									<?php wp_nonce_field( 'export_supplier_action', 'export_supplier_nonce' ); ?>
									<button type="submit"
