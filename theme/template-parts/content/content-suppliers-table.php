<?php
/**
 * Template part for displaying suppliers table with client-side pagination and filtering
 *
...
$all_suppliers = $wpdb->get_results( $query );
...
foreach ( $all_suppliers as $supplier ) {
    $visit_date    = ! empty( $supplier->visit_date ) ? date( 'M j, Y', strtotime( $supplier->visit_date ) ) : 'N/A';
    $sign_in_time  = ! empty( $supplier->sign_in_time ) ? date( 'g:i a', strtotime( $supplier->sign_in_time ) ) : null;
    $sign_out_time = ! empty( $supplier->sign_out_time ) ? date( 'g:i a', strtotime( $supplier->sign_out_time ) ) : null;

    $current_date          = current_time( 'Y-m-d' );
    $normalized_visit_date = substr( $supplier->visit_date ?? '', 0, 10 );
    $visit_status          = strtolower( $supplier->status ?? 'approved' );
    ...
    if ( $normalized_visit_date && $normalized_visit_date > $current_date ) {
        ...
        $visit_status = ! empty( $supplier->sign_in_time ) ? ( ! empty( $supplier->sign_out_time ) ? 'completed' : 'signout' ) : 'signin';
    } elseif ( $normalized_visit_date && $normalized_visit_date < $current_date ) {
        $visit_status = ! empty( $supplier->sign_in_time ) ? ( ! empty( $supplier->sign_out_time ) ? 'completed' : 'signout' ) : 'missed';
    }

    $suppliers_data[] = array(
        'id'              => $supplier->id,
        'visit_id'        => $supplier->visit_id,
        'first_name'      => $supplier->first_name,
        'last_name'       => $supplier->last_name,
        'id_number'       => $supplier->id_number ?? 'N/A',
        'email'           => $supplier->email ?? '',
        'phone_number'    => $supplier->phone_number ?? '',
        'visit_status'    => $supplier->visit_status,
        'status_class'    => $status_classes[ $supplier->visit_status ] ?? $status_classes['approved'],
...
<div x-data="supplierTable()" x-init="init()"
    @search-suppliers-updated.window="searchTerm = $event.detail; performSearch()"
    @clear-suppliers-search.window="clearSearch()">
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div x-show="filteredSuppliers.length > 0"
            data-supplier-details-url="<?php echo esc_url( home_url( '/supplier-details' ) ); ?>"
            class="max-w-full overflow-x-auto" id="suppliers-table">
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <th class="px-3 py-3 sm:px-6">
                            <div class="flex items-center">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">First Name
                                </p>
                            </div>
                        </th>
                        <th class="px-3 py-3 sm:px-6">
                            <div class="flex items-center">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Last Name
                                </p>
                            </div>
                        </th>
                        <th class="px-3 py-3 sm:px-6">
                        ...
                        <th class="px-3 py-3 sm:px-6">
                            <div class="flex items-center">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">ID Number
                                </p>
                            </div>
                        </th>
                        <th class="px-3 py-3 sm:px-6">
                            <div class="flex items-center">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Visit Date
                                </p>
                            </div>
                        </th>
                        <th class="px-3 py-3 sm:px-6">
                            <div class="flex items-center">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Actions
                                </p>
                            </div>
                        </th>
                    </tr>
                </thead>
                <tbody id="suppliers-table-body" class="divide-y divide-gray-100 dark:divide-gray-800">
                    <template x-for="(supplier, index) in paginatedSuppliers" :key="supplier.visit_id">
                        <tr :data-guest-id="supplier.id" :data-visit-id="supplier.visit_id">
                            ...
                            <td class="px-3 py-4 sm:px-6">
                                <div class="flex items-center">
                                    <span
                                        ...
                                        :class="supplier.status_class" x-text="supplier.visit_status"></span>
                                </div>
                            </td>
                            <td class="px-3 py-4 sm:px-6">
                                <div class="flex items-center">
                                    <p class="id_number text-gray-500 text-theme-sm dark:text-gray-400"
                                        x-text="supplier.id_number"></p>
                                </div>
                            </td>
                            <td class="px-3 py-4 sm:px-6">
                                <div class="flex items-center">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400"
                                        x-text="supplier.visit_date">
                                    </p>
                                </div>
                            </td>
                            <td class="px-3 py-4 sm:px-6">
                                <div class="flex items-center gap-2">
                                    <button :id="'edit-supplier-button-' + supplier.id"
                                        ...
                                        :data-guest-id="supplier.id" :data-visit-id="supplier.visit_id">
                                    ...
                                    <template x-if="supplier.visit_status === 'cancelled'">
                                        <span
                                    ...
                                    <template x-if="supplier.visit_status === 'unapproved'">
                                        <span
                                    ...
                                    <template x-if="supplier.visit_status === 'suspended'">
                                        <span
                                    ...
                                    <template x-if="supplier.visit_status === 'banned'">
                                        <span
                                    ...
                                    <template x-if="supplier.visit_status === 'approved'">
                                        <div>
                                            <template x-if="supplier.computed_status === 'missed'">
                                                <span
                                            ...
                                            <template x-if="supplier.computed_status === 'scheduled'">
                                                <span
                                            ...
                                            <template x-if="supplier.computed_status === 'signin'">
                                                <button :id="'sign-in-supplier-button-' + supplier.id"
                                                    ...
                                                    :data-visit-id="supplier.visit_id" :disabled="isMemberOrChairman">
                                                    Sign In
                                                </button>
                                            </template>

                                            <template x-if="supplier.computed_status === 'signout'">
                                                <button :id="'sign-out-supplier-button-' + supplier.id"
                                            ...
                                            <template x-if="supplier.computed_status === 'completed'">
                                                <div class="flex flex-col items-center justify-center text-xs px-4">
                                                    <span class="text-green-600 dark:text-green-400"
                                                        x-text="supplier.sign_in_time"></span>
                                                    <span class="text-red-600 dark:text-red-400"
                                                        x-text="supplier.sign_out_time"></span>
                                                </div>
                                            </template>
                                        </div>
                                    </template>
        ...
        <!-- Empty state message -->
        <div x-show="filteredSuppliers.length === 0" class="p-8 text-center">
            <template x-if="searchTerm">
                <div class="py-8">
                    ...
                    <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No suppliers found</h3>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        No suppliers match your search for "<span x-text="searchTerm" class="font-medium"></span>".
                    </p>
            ...
            <template x-if="!searchTerm && allSuppliers.length === 0">
                <div class="py-8">
                    <svg class="w-16 h-16 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                    </svg>
                    <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No supplier visits yet</h3>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        Get started by registering a new supplier visit.
                    </p>
                </div>
            </template>
        </div>
        ...
        <div x-show="filteredSuppliers.length > 0 && totalPages > 1"
...
function supplierTable() {
    return {
        allSuppliers: <?php echo !empty($suppliers_data) ? json_encode($suppliers_data) : '[]'; ?>,
        perPage: 20,
        currentPage: 1,
        searchTerm: '',
        ...
        init() {
            const savedPerPage = localStorage.getItem('suppliers_per_page');
            const savedPage = localStorage.getItem('suppliers_current_page');
            ...
            // Check if there's a search term in URL - use consistent parameter name
            const urlParams = new URLSearchParams(window.location.search);
            const searchParam = urlParams.get('user_search'); // Keep this consistent
            if (searchParam) {
        ...
        get filteredSuppliers() {
            ...
            return this.allSuppliers.filter(supplier => {
                // Check if supplier exists and has properties
                if (!supplier) return false;

                return (
                    (supplier.first_name && supplier.first_name.toLowerCase().includes(searchTerm)) ||
                    (supplier.last_name && supplier.last_name.toLowerCase().includes(searchTerm)) ||
                    (supplier.id_number && supplier.id_number.toLowerCase().includes(searchTerm)) ||
                    (supplier.email && supplier.email.toLowerCase().includes(searchTerm)) ||
                    (supplier.phone_number && supplier.phone_number.includes(searchTerm)) ||
                    (supplier.visit_status && supplier.visit_status.toLowerCase().includes(
                        searchTerm)) ||
                    (supplier.computed_status && supplier.computed_status.toLowerCase().includes(
                        searchTerm))
                );
            });
        },
        ...
        updatePerPage() {
            localStorage.setItem('suppliers_per_page', this.perPage);
            this.currentPage = 1;
            localStorage.setItem('suppliers_current_page', this.currentPage);
        },
        ...
        performSearch() {
            ...
            if (this.searchTerm) {
                url.searchParams.set('user_search', this.searchTerm); // Keep consistent
                url.searchParams.set('search_users', 'true'); // Keep consistent
            } else {
        ...
        clearSearch() {
            ...
            const url = new URL(window.location);
            url.searchParams.delete('user_search'); // Keep consistent
            url.searchParams.delete('search_users'); // Keep consistent
            window.history.pushState({}, '', url);
...
// Keyboard shortcuts - Keep only one instance
