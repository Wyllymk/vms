...
<div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]"
    x-data="supplierTable()" x-init="init()">
    ...
    <div class="max-w-full overflow-x-auto" id="suppliers-table"
        ...
        <table class="min-w-full">
            ...
            <tbody id="suppliers-table-body" class="divide-y divide-gray-100 dark:divide-gray-800">
                <template x-if="paginatedSuppliers.length === 0">
                    <tr>
                        <td colspan="8" class="px-4 py-4 text-center text-gray-600 dark:text-white">No suppliers found.
                        </td>
                    </tr>
                </template>
                ...
                <template x-for="(supplier, index) in paginatedSuppliers" :key="supplier.visit_id">
                    <tr :data-guest-id="supplier.id" :data-visit-id="supplier.visit_id">
                        ...
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-800 text-theme-sm dark:text-white/90" x-text="supplier.first_name">
                                </p>
                            </div>
                        </td>
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-800 text-theme-sm dark:text-white/90" x-text="supplier.last_name">
                                </p>
                            </div>
                        </td>
                        <td class="px-3 py-4 sm:px-6">
                        ...
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="supplier.visit_date">
                                </p>
                            </div>
                        </td>
                        <td class="px-3 py-4 sm:px-6">
...
function supplierTable() {
    return {
        allSuppliers: <?php echo json_encode( $suppliers_data ); ?>,
        perPage: 25,
        currentPage: 1,
        ...
        get totalPages() {
            return Math.ceil(this.allSuppliers.length / this.perPage);
        },

        get paginatedSuppliers() {
            const start = (this.currentPage - 1) * this.perPage;
            const end = start + this.perPage;
            return this.allSuppliers.slice(start, end);
        },
        ...
        getEntriesText() {
            const start = this.allSuppliers.length > 0 ? ((this.currentPage - 1) * this.perPage) + 1 : 0;
            const end = Math.min(this.currentPage * this.perPage, this.allSuppliers.length);
            const total = this.allSuppliers.length;
