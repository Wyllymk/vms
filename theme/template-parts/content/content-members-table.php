<?php
/**
 * Template part for displaying employees/users table with client-side pagination and filtering
 *
...
foreach ( $users as $user ) {
    ...
    $users_data[] = array(
        ...
        'details_url'        => esc_url( add_query_arg( 'user_id', $user_id, home_url( '/details' ) ) ),
    );
}
?>
            ...
            <tbody id="users-table-body" class="divide-y divide-gray-100 dark:divide-gray-800">
                <template x-for="(user, index) in paginatedUsers" :key="user.id">
                    <tr>
                        ...
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="user.first_name"></p>
                            </div>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="user.last_name"></p>
                            </div>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex items-center">
                        ...
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="user.phone_number">
                                </p>
                            </div>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex items-center">
                                <span
                                    ...
                                    :class="user.status_class" x-text="user.registration_status"></span>
                            </div>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <form :action="user.details_url" method="get">
...
function usersTable() {
    return {
        ...
        init() {
            ...
            const savedPage = localStorage.getItem('users_current_page');
        ...
        updatePerPage() {
            localStorage.setItem('users_per_page', this.perPage);
            this.currentPage = 1;
