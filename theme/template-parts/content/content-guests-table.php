<?php
/**
 * Template part for displaying guests table with client-side pagination and filtering
 *
...
foreach ( $all_guests as $guest ) {
    ...
    $host_name      = ! empty( $full_host_name ) ? $full_host_name : ( $guest->display_name ?? 'N/A' );
...
<div x-data="guestTable()" x-init="init()" @search-guests-updated.window="searchTerm = $event.detail; performSearch()"
    @clear-guests-search.window="clearSearch()"
    ...
    <div x-show="filteredGuests.length > 0" class="max-w-full overflow-x-auto" id="guests-table"
        ...
        <table class="min-w-full">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <th class="px-3 py-3 sm:px-6">
                        <div class="flex items-center">
                            <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Host Member</p>
                        </div>
                    </th>
                    <th class="px-3 py-3 sm:px-6">
                        <div class="flex items-center">
            ...
            <tbody id="guests-table-body" class="divide-y divide-gray-100 dark:divide-gray-800">
                <template x-for="(guest, index) in paginatedGuests" :key="guest.visit_id">
                    <tr :data-guest-id="guest.id" :data-visit-id="guest.visit_id">
                        ...
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <template x-if="guest.is_courtesy">
                                    <span
                                ...
                                <template x-if="!guest.is_courtesy">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="guest.host_name">
                                    </p>
                                </template>
                            </div>
                        </td>
    ...
    <div x-show="filteredGuests.length === 0" class="p-8 text-center">
        <template x-if="searchTerm">
            <div class="py-8">
                ...
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                    No guests match your search for "<span x-text="searchTerm" class="font-medium"></span>".
                </p>
        ...
        <template x-if="!searchTerm && allGuests.length === 0">
            <div class="py-8">
                ...
                <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No guest visits yet</h3>
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                    Get started by registering a new guest visit.
                </p>
            </div>
        </template>
    </div>
...
function guestTable() {
    return {
        ...
        currentPage: 1,
        searchTerm: '',
        isMemberOrChairman: <?php echo $is_member_or_chairman ? 'true' : 'false'; ?>,

        init() {
            const savedPerPage = localStorage.getItem('guests_per_page');
            const savedPage = localStorage.getItem('guests_current_page');
        ...
        get filteredGuests() {
            ...
            return this.allGuests.filter(guest => {
                ...
                return (
                    ...
                    (guest.host_name && guest.host_name.toLowerCase().includes(searchTerm)) ||
        ...
        updatePerPage() {
            localStorage.setItem('guests_per_page', this.perPage);
            this.currentPage = 1;
            localStorage.setItem('guests_current_page', this.currentPage);
        },
...
// Add keyboard shortcut for search (only once globally)
if (typeof window.guestsSearchKeyboardShortcuts === 'undefined') {
    window.guestsSearchKeyboardShortcuts = true;
    document.addEventListener('DOMContentLoaded', function() {
        document.addEventListener('keydown', function(e) {
            ...
            // Escape to clear search - dispatch the clear event
            if (e.key === 'Escape') {
                const searchInput = document.getElementById('search-guests-input');
                if (searchInput && searchInput.value) {
                    ...
                    window.dispatchEvent(new CustomEvent('clear-guests-search'));
                }
            }
        });
    });
