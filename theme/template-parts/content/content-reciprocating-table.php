<?php
/**
 * Template part for displaying reciprocating members table with client-side pagination and filtering
 *
...
$all_members = $wpdb->get_results( $query );
...
foreach ( $all_members as $member ) {
    $visit_date    = ! empty( $member->visit_date ) ? date( 'M j, Y', strtotime( $member->visit_date ) ) : 'N/A';
    $sign_in_time  = ! empty( $member->sign_in_time ) ? date( 'g:i a', strtotime( $member->sign_in_time ) ) : null;
    $sign_out_time = ! empty( $member->sign_out_time ) ? date( 'g:i a', strtotime( $member->sign_out_time ) ) : null;

    $current_date          = current_time( 'Y-m-d' );
    $normalized_visit_date = substr( $member->visit_date ?? '', 0, 10 );
    $visit_status          = strtolower( $member->status ?? 'approved' );
    ...
    $members_data[] = array(
        ...
        'status_class'        => $status_classes[ $member->visit_status ] ?? $status_classes['approved'],
        ...
        <table class="min-w-full">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <th class="px-3 py-3 sm:px-6">
                        <div class="flex items-center">
                            <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Club</p>
                        </div>
                    </th>
                    <th class="px-3 py-3 sm:px-6">
                        <div class="flex items-center">
                            <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Member Number</p>
                        </div>
                    </th>
                    <th class="px-3 py-3 sm:px-6">
                        <div class="flex items-center">
            ...
            <tbody id="reciprocating-members-table-body" class="divide-y divide-gray-100 dark:divide-gray-800">
                <template x-for="(member, index) in paginatedMembers" :key="member.visit_id">
                    <tr :data-member-id="member.id" :data-visit-id="member.visit_id">
                        ...
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-800 text-theme-sm dark:text-white/90" x-text="member.first_name">
                                </p>
                            </div>
                        </td>
                        <td class="px-3 py-4 sm:px-6">
                        ...
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <span
                                    ...
                                    :class="member.status_class" x-text="member.visit_status"></span>
                            </div>
                        </td>
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                        ...
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400"
                                    x-text="member.reciprocating_member_number"></p>
                            </div>
                        </td>
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="member.visit_date">
                                </p>
                            </div>
                        </td>
                        <td class="px-3 py-4 sm:px-6">
                            <div class="flex items-center gap-2">
                                <button :id="'edit-reciprocating-member-button-' + member.id"
                                    ...
                                    :data-member-id="member.id" :data-visit-id="member.visit_id">
                                ...
                                <template x-if="member.visit_status === 'cancelled'">
                                    <span
                                ...
                                <template x-if="member.visit_status === 'unapproved'">
                                    <span
                                ...
                                <template x-if="member.visit_status === 'suspended'">
                                    <span
                                ...
                                <template x-if="member.visit_status === 'banned'">
                                    <span
                                ...
                                <template x-if="member.visit_status === 'approved'">
                                    <div>
                                        <template x-if="member.computed_status === 'missed'">
                                            <span
                                        ...
                                        <template x-if="member.computed_status === 'scheduled'">
                                            <span
                                        ...
                                        <template x-if="member.computed_status === 'signin'">
                                            <button :id="'reciprocating-sign-in-button-' + member.id"
                                        ...
                                        <template x-if="member.computed_status === 'signout'">
                                            <button :id="'reciprocating-sign-out-button-' + member.id"
                                                ...
                                                :data-visit-id="member.visit_id">
                                                Sign Out
                                            </button>
                                        </template>

                                        <template x-if="member.computed_status === 'completed'">
                                            <div class="flex flex-col items-center justify-center text-xs px-4">
                                                <span class="text-green-600 dark:text-green-400"
                                                    x-text="member.sign_in_time"></span>
                                                <span class="text-red-600 dark:text-red-400"
                                                    x-text="member.sign_out_time"></span>
                                            </div>
                                        </template>
                                    </div>
                                </template>
...
function memberTable() {
    return {
        ...
        init() {
            ...
            const savedPage = localStorage.getItem('members_current_page');
        ...
        updatePerPage() {
            localStorage.setItem('members_per_page', this.perPage);
            this.currentPage = 1;
