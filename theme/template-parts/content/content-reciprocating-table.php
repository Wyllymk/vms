 <?php
/**
 * Template part for displaying reciprocating members table with pagination
 *
...
if ( isset( $_GET['search_users'] ) && ! empty( $_GET['user_search'] )  ) {
    ...
    // For visits count search - need to join with members table
...
// Count total member visits with role filtering
...
// Fetch visits with role filtering
$query = "SELECT 
            m.*, 
            v.id AS visit_id, 
            v.visit_date, 
                 ...
                 <tr data-member-id="<?php echo esc_attr( $member->id ); ?>"
                     ...
                     <td class="px-3 py-4 sm:px-6">
                         <div class="flex items-center">
                             <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                 <?php
                                echo ! empty( $member->reciprocating_member_number )
